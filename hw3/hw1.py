...
def check_data(filepath: str, validators: Iterable[Callable]) -> str:
    ...
    with open(filepath, "r", encoding="utf-8") as f, open(report_path, "w", encoding="utf-8") as report:
        for line in f:
            errors = []
            fields = line.strip().split()
            if not validate_line(line):
                errors.append("validate_line")
            if not validate_date(fields[-1]):
                errors.append("validate_date")
            if errors:
                error_reason = errors[0]
                report.write(f"{line.strip()} {error_reason}\n")
    return report_path